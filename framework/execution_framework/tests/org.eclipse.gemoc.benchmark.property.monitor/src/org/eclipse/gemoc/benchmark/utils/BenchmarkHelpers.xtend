...
class BenchmarkHelpers {
	...
	static def void copyFileInWS(File file, IFolder destination, IProgressMonitor m) {
	...
	static def IFolder copyFolderInWS(File folder, IResource destination, IProgressMonitor m) {
	...
	static def IFolder copySingleFileInWS(File folder, String filename, IResource destination, IProgressMonitor m) {
		val folderCopy = if (destination instanceof IProject) {
				destination.getFolder(folder.name)
			} else if (destination instanceof IFolder) {
				destination.getFolder(folder.name)
	...
	static def createEmptyPrintStream() {
	...
	static def void clearResourceSet(ResourceSet rs) {
	...
	static def <T,U, V extends Collection<U>> Map<T, V> mergeMaps(Map<T, V>... maps) {
