...
public class MelangeHelper {
	...
	/**
	 * @param languageId
	 * @return the URI of the given language or null if no such language exists in
	 *         the registry
	 */
	...
	/**
	 * Note: in some situations, the same uri can be declared for several languages
	 * ! (for example using external keyword) this is usually a deployment issue
	 * 
	 * @param uri
	 * @return the first language with the given URI or null if no such language
	...
	/**
	 * Find in the given resource the native languages corresponding to the element
	 * at the root of the resource (built as a set in order to have no duplicate)
	 * 
	 * @param res
	 * @return a list of all the native languages of the root elements of the
	 *         resource
	 */
	...
	/**
	 * Search in 'aspect' for a method named 'calledMethodName' and with one
	 * parameter. The type of this parameter has the same simple name as the EClass
	 * of 'caller'.
	 * 
	...
	public static Method findMethod(Class<?> aspect, EObject caller, String calledMethodName) throws NoSuchMethodException {
		...
		for (Method method : aspect.getMethods()) {
			...
			if (methodName.equals(calledMethodName) && paramTypes.length == 1 && callerClassNames.stream().anyMatch(s -> paramTypes[0].getSimpleName().equals(s))) {
				return method;
			}
		}
	...
	public static Method findMethod(Class<?> aspect, EObject caller, Class<?>[] parameterTypes, String calledMethodName) throws NoSuchMethodException {
		...
		for (Method method : aspect.getMethods()) {
			String methodName = method.getName();
			Class<?>[] paramTypes = method.getParameterTypes();
			if (methodName.equals(calledMethodName) && paramTypes.length == parameterTypes.length + 1 && callerClassNames.stream().anyMatch(s -> paramTypes[0].getSimpleName().equals(s))) {
				boolean found = true;
