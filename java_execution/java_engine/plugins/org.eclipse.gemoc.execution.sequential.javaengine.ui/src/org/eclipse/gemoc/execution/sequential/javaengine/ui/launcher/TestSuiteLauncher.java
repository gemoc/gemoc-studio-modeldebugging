...
public class TestSuiteLauncher extends AbstractGemocLauncher {
	...
	private ILaunchConfiguration getLaunchConfiguration(TestCase testCase, String languageName) throws CoreException {
		...
		configuration.setAttribute(RunConfiguration.LAUNCH_SELECTED_LANGUAGE, languageName);
		...
		try {
			...
			for (TestCase testCase : testSuite.getTestCases()) {
				try {
					...
					final RunConfiguration runConf = new RunConfiguration(launchConf);
					final IExecutionEngine engine = createExecutionEngine(runConf, ExecutionMode.Run);
					final IEventManager eventManager = engine.getAddon(EventManager.class);
					final List<Event> events = new ArrayList<>(testCase.getScenario());
